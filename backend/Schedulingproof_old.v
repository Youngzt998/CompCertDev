...
(* i1 i2 have dependence, order irrelevant *)
...
(* Matching relation between register relations *)
Section RS_AGREE.
...
| match_call_state:
    ...
    (STK: match_stack sl sl')
    (FUNC: f = f')        (** TODO: maybe too tough? can we make sure function pointer values are kept during compilation ?  - though not hard to modify to a looser one *)
...
Section SINGLE_SWAP_CORRECTNESS.
  ...
  Lemma return_address_offset_determ:
    forall f c ra ra', return_address_offset f c ra -> return_address_offset f c ra' ->
      ra = ra'.
  Proof. intros. repeat red in H, H0.

  Lemma mach_determ: forall rao, determinate (Mach.semantics rao tprog).
  Proof. 
    constructor.
    - intros. inv H; inv H0.
      ...
      + split. eapply match_traces_E0. intros; auto. rewrite H12 in H1; inv H1; auto.
      + split. eapply match_traces_E0. intros; auto. rewrite H16 in H3; inv H3; auto.
      + split. eapply match_traces_E0. intros; auto. rewrite H12 in H1; inv H1; auto.
      + split. eapply match_traces_E0. intros; auto.
  ...
  Lemma independ_two_step_match:
    forall sl sl' fb fb' sp sp' c rs1 rs1' m1 m1' s3 s3' i1 i2 t t'
    (INDEP: i1 D~> i2 = false)
    (s1:= State sl fb sp (i1 :: i2 :: c) rs1 m1)
    (STEP13: starN step ge 2 s1 t s3)
    (s1':= State sl' fb' sp' (i2 :: i1 :: c) rs1' m1')
    (STEP13': starN step ge 2 s1' t' s3')
...
Admitted.
  ...
  forall s2, match_states s1 s2 ->
    exists n s2', tplus s2 t s2' /\ tEventually n s1' (fun s1'' => match_states s1'' s2').
  Proof. 
      ...
      { destruct n.
        ...
        + simpl. remember (i1 D~> i2) as INDEP. 
          ...
          { exists 0%nat. inv MEM.
            ...
            destruct H0.
            exists x. split. eapply plus_one. eapply H0. eapply eventually_now; auto.  
          }
          (* swap succeeded, take two independent steps, from previous lemma  *)
          { exists 1%nat. inv MEM. simpl in WF'. rewrite HeqINDEP in WF'.
            edestruct independ_two_step_match; eauto.
