  ...
  for (t2=0;t2<=floord(_PB_N-1,32);t2++) {
    for (t3=0;t3<=floord(_PB_M-1,32);t3++) {
      for (t4=32*t2;t4<=min(_PB_N-1,32*t2+31)-1;t4+=2) {
        DATA_TYPE __r0 = q[t4];
        DATA_TYPE __r1 = q[(t4+1)];
        for (t5=32*t3;t5<=min(_PB_M-1,32*t3+31)-1;t5+=2) {
          __r0 = __r0 + A[t4][t5] * p[t5];;
          __r0 = __r0 + A[t4][(t5+1)] * p[(t5+1)];;
          __r1 = __r1 + A[(t4+1)][t5] * p[t5];;
          __r1 = __r1 + A[(t4+1)][(t5+1)] * p[(t5+1)];;
        }
        q[t4] = __r0;
        q[(t4+1)] = __r1;
      }
  ...
  for (t2=0;t2<=floord(_PB_M-1,32);t2++) {
    for (t3=0;t3<=floord(_PB_N-1,32);t3++) {
      for (t4=32*t3;t4<=min(_PB_N-1,32*t3+31)-1;t4+=2) {
        DATA_TYPE __r0 = r[t4];
        DATA_TYPE __r1 = r[(t4+1)];
        for (t5=32*t2;t5<=min(_PB_M-1,32*t2+31)-1;t5+=2) {
          s[t5] = s[t5] + __r0 * A[t4][t5];;
          s[(t5+1)] = s[(t5+1)] + __r0 * A[t4][(t5+1)];;
          s[t5] = s[t5] + __r1 * A[(t4+1)][t5];;
          s[(t5+1)] = s[(t5+1)] + __r1 * A[(t4+1)][(t5+1)];;
        }
      }
    }
  }
