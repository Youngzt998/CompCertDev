...
if (_PB_N >= 1) {
  for (t1=0;t1<=floord(_PB_N-1,32);t1++) {
    for (t2=0;t2<=floord(_PB_N-1,32);t2++) {
      for (t3=32*t1;t3<=min(_PB_N-1,32*t1+31)-1;t3+=2) {
        DATA_TYPE __r0 = x1[t3];
        DATA_TYPE __r1 = x2[t3];
        DATA_TYPE __r2 = x1[(t3+1)];
        DATA_TYPE __r3 = x2[(t3+1)];
        for (t4=32*t2;t4<=min(_PB_N-1,32*t2+31)-1;t4+=2) {
          __r0 = __r0 + A[t3][t4] * y_1[t4];;
          __r1 = __r1 + A[t4][t3] * y_2[t4];;
          __r0 = __r0 + A[t3][(t4+1)] * y_1[(t4+1)];;
          __r1 = __r1 + A[(t4+1)][t3] * y_2[(t4+1)];;
          __r2 = __r2 + A[(t3+1)][t4] * y_1[t4];;
          __r3 = __r3 + A[t4][(t3+1)] * y_2[t4];;
          __r2 = __r2 + A[(t3+1)][(t4+1)] * y_1[(t4+1)];;
          __r3 = __r3 + A[(t4+1)][(t3+1)] * y_2[(t4+1)];;
        }
        x1[t3] = __r0;
        x2[t3] = __r1;
        x1[(t3+1)] = __r2;
