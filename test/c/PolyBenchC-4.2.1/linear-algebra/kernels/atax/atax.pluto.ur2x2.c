...
if (_PB_N >= 1) {
  for (t2=0;t2<=floord(_PB_M-1,32);t2++) {
    for (t3=0;t3<=floord(_PB_N-1,32);t3++) {
      for (t4=32*t2;t4<=min(_PB_M-1,32*t2+31)-1;t4+=2) {
        DATA_TYPE __r0 = tmp[t4];
        DATA_TYPE __r1 = tmp[(t4+1)];
        for (t5=32*t3;t5<=min(_PB_N-1,32*t3+31)-1;t5+=2) {
          __r0 = __r0 + A[t4][t5] * x[t5];;
          __r0 = __r0 + A[t4][(t5+1)] * x[(t5+1)];;
          __r1 = __r1 + A[(t4+1)][t5] * x[t5];;
          __r1 = __r1 + A[(t4+1)][(t5+1)] * x[(t5+1)];;
        }
        tmp[t4] = __r0;
        tmp[(t4+1)] = __r1;
      }
...
if (_PB_M >= 1) {
  for (t2=0;t2<=floord(_PB_N-1,32);t2++) {
    for (t3=0;t3<=floord(_PB_M-1,32);t3++) {
      for (t4=32*t3;t4<=min(_PB_M-1,32*t3+31)-1;t4+=2) {
        DATA_TYPE __r0 = tmp[t4];
        DATA_TYPE __r1 = tmp[(t4+1)];
        for (t5=32*t2;t5<=min(_PB_N-1,32*t2+31)-1;t5+=2) {
          y[t5] = y[t5] + A[t4][t5] * __r0;;
          y[(t5+1)] = y[(t5+1)] + A[t4][(t5+1)] * __r0;;
          y[t5] = y[t5] + A[(t4+1)][t5] * __r1;;
          y[(t5+1)] = y[(t5+1)] + A[(t4+1)][(t5+1)] * __r1;;
        }
      }
    }
  }
