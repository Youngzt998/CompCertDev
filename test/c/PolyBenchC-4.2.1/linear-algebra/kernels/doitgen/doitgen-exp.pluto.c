    ...
    for (t3=0;t3<=floord(_PB_NQ-1,32);t3++) {
      for (t4=0;t4<=floord(_PB_NP-1,32);t4++) {
        for (t5=0;t5<=floord(_PB_NP-1,32);t5++) {
          // unroll(2,2,2,1) dtype(DATA_TYPE)
          for (t6=32*t2;t6<=min(_PB_NR-1,32*t2+31);t6++) {
            for (t7=32*t3;t7<=min(_PB_NQ-1,32*t3+31);t7++) {
              for (t8=32*t4;t8<=min(_PB_NP-1,32*t4+31);t8++) {
                for (t9=32*t5;t9<=min(_PB_NP-1,32*t5+31);t9++) {
                  sum2[t6][t7][t8] += A[t6][t7][t9] * C4[t9][t8];;
                }
              }
            }
          }
